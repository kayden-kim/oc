...
func buildEditChoices(paths runtimePaths, projectConfigPath string, projectConfigExists bool) []tui.EditChoice {
	...
	for _, path := range userOhMyConfigs {
		choices = append(choices, tui.EditChoice{
			Label: fmt.Sprintf("%d) %s file", len(choices)+1, filepath.Base(path)),
			Path:  path,
		})
	}
	...
	if projectConfigExists {
		choices = append(choices, tui.EditChoice{
			Label: fmt.Sprintf("%d) project opencode.json file", len(choices)+1),
			Path:  projectConfigPath,
		})

		for _, path := range DiscoverOhMyConfigPaths(filepath.Dir(projectConfigPath)) {
			choices = append(choices, tui.EditChoice{
				Label: fmt.Sprintf("%d) project %s file", len(choices)+1, filepath.Base(path)),
				Path:  path,
			})
		}
	}
